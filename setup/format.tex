...
\newcommand{\xiaosi}{\fontsize{12pt}{12pt}\selectfont}  	% 小四, 1.倍行距 	modified by Jon, 小四对应12pt
...
% modified by Jon，此处用于设置正文字体
\makeatletter
\renewcommand\normalsize{
  \@setfontsize\normalsize{12pt}{11.5pt} 	% modified by Jon, 正文使用小四号(12pt)字体
...
% modified by Jon, 对目录的显示格式进行调整，目录中的项需要使用 宋体4号
%%%%%%%%%%%%% Contents %%%%%%%%%%%%%%%%%
...
\def\cschoolnumer#1{\def\@cschoolnumer{#1}}\def\@cschoolnumer{}
\def\ccollegename#1{\def\@ccollegename{#1}}\def\@ccollegename{}
...
\def\makepageheader{
	...
	\fancypagestyle{plain}{% 设置页页眉页脚的字体
	    \fancyhf{}%
	\fancyhead[CO]{\song\wuhao \@ctitle}		% modify by Jon: 奇数页的页眉标题也使用@ctitle
