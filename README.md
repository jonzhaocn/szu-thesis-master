...
> 该代码（建议）来源：[这里](https://tex.stackexchange.com/questions/247457/how-do-i-remove-header-and-footer-from-contents-page-and-list-of-figures)

目录之前的其他页面使用到了特定的页脚页眉格式，在编写文档的过程中发现，当目录多余1页时，使用`\thispagestyle{empty}`和`\pagestyle{empty}`，都无法完全清除目录的页眉页脚格式。

后来搜索发现，需要使用到下面的`\AtBeginDocument{}`代码进行设置。
...
当使用pdf阅读器（如Adobe Acrobat Reader）打开`main.pdf`的时候，编译main.tex文件时，会出现无法写入的错误

```
I can't write on file `main.pdf`. ...
```

这个时候需要关闭pdf文件后，才可以正常编译。若使用`Texstudio`等编辑器，则可以做到同时编译与查看pdf文件。